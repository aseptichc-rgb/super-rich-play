...
const when=s=>`Year ${Math.floor(s.month/12)+1} · Month ${s.month%12+1}`;
const SYNCED='✓ Saved · Cloud synced',PENDING='✓ Saved · Cloud sync pending',FAILED='Saved locally · Cloud sync failed',CHOOSE='Saved locally · Choose a save to keep',LOCAL='✓ Saved';
...
export function createCloudUI({client,storage,version,getState,replaceState,openDialog,closeDialog,toast,setStatus,showSettings,scenarioName,timers={set:(fn,ms)=>setTimeout(fn,ms),clear:id=>clearTimeout(id)}}){
 ...
 async function push(){
  ...
  busy=true;again=false;timers.clear(timer);setStatus('Syncing…');
  ...
  if(res.revision==null){stop('This cloud save was deleted on another device, so syncing stopped. Your game is still saved here.');return;}
  ask();
 }
 ...
 async function compare(row){
  ...
  if(!row){const r=await client.load(code);if(memo?.code!==code)return;if(!r.ok){setStatus(FAILED);toast('Could not reach the cloud. Try again in a moment.');return;}if(!r.row){stop('This cloud save no longer exists, so syncing stopped. Your game is still saved here.');return;}row=r.row;}
  pending={row,save:remoteSave(row.data),code};showCompare();
 }
 const side=(title,s,extra='')=>`<section class="cloud-side"><h3>${title}</h3>${s?`<p><b>${when(s)}</b></p><p>Net worth ${money(analyze(s).wealth)}</p><p>${esc(scenarioName(s))}</p>`:'<p role="alert">This save could not be read.</p>'}${extra}</section>`;
 function showCompare(){
  const {row,save}=pending,updated=row.updated_at?new Date(row.updated_at).toLocaleString('en-US'):'';
  openDialog('cloud-compare',`<span class="eyebrow">CLOUD SAVE</span><h2>Which save do you want to keep?</h2><p>This device and your cloud save have both moved on. The one you don't pick is replaced.</p><div class="cloud-compare">${side('This Device',getState())}${side('Cloud Save',save,updated?`<p class="help">Saved ${esc(updated)}</p>`:'')}</div><div class="button-row"><button class="primary" data-cloud="keep-local">Keep This Device's Save</button><button data-cloud="use-remote" ${save?'':'disabled'}>Load Cloud Save</button></div><button class="full" data-action="export">Export This Device's Save First</button>`);
 }
 function useRemote(){
  ...
  setStatus(SYNCED);toast('Continued from your cloud save.');
 }
 ...
 async function create(){
  ...
  busy=false;toast('Could not reach the cloud. Your game is still saved on this device.');
 }
 function showCode(code){openDialog('cloud-code',`<span class="eyebrow">CLOUD SAVE</span><h2>Your save code</h2><p class="cloud-code">${formatCode(code)}</p><p>On another device, open Settings → Cloud Save and enter this code to continue there.</p><p role="alert">Copy it or write it down now. A lost code can't be recovered, and anyone who has it can open, overwrite or delete this save.</p><button class="primary full" data-cloud="copy">Copy Code</button>`);}
 async function loadCode(){
  const code=normalizeCode(document.querySelector('#cloud-code-input')?.value);
  if(!code){toast('Enter the 16-character save code, like K7QM-3XRA-94TD-PW2E.');return;}
  if(busy)return;
  busy=true;const r=await client.load(code);busy=false;
  if(!r.ok){toast('Could not reach the cloud. Try again in a moment.');return;}
  if(!r.row){toast('No cloud save matches that code.');return;}
  const save=remoteSave(r.row.data);
  if(!save){toast('That cloud save could not be read. Your game here is unchanged.');return;}
 ...
 function copy(){
  if(!memo)return;
  const text=formatCode(memo.code),fail=()=>toast('Copy failed. Write the code down: '+text);
  if(navigator.clipboard?.writeText)navigator.clipboard.writeText(text).then(()=>toast('Save code copied.'),fail);else fail();
 }
 async function remove(){
  if(!memo||busy)return;
  busy=true;const r=await client.remove(memo.code);busy=false;
  if(!r.ok){toast('Could not reach the cloud. Nothing was deleted.');return;}
  stop('Cloud save deleted. Your game is still saved on this device.');closeDialog();showSettings();
 }
 document.addEventListener('click',e=>{
  ...
  else if(a==='stop'){stop('Syncing stopped on this device. Your cloud save is kept.');showSettings();}
  else if(a==='delete')openDialog('cloud-delete','<h2>Delete this cloud save?</h2><p>The save code stops working on every device. The game saved on this device stays.</p><div class="button-row"><button class="primary" data-cloud="delete-confirm">Delete Cloud Save</button><button data-action="close">Cancel</button></div>');
  else if(a==='delete-confirm')remove();
 });
 return{
  settingsHTML(){
   if(!memo)return`<h3>Cloud Save</h3><p class="help">Continue this game on another device with a save code. No sign-up needed. Anyone with the code can open, overwrite or delete the save, and a lost code can't be recovered.</p><button data-cloud="create" class="full">Save to Cloud</button><label class="field-label">Already have a code?<input id="cloud-code-input" maxlength="24" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="XXXX-XXXX-XXXX-XXXX"></label><button data-cloud="load-code">Continue With a Save Code</button>`;
   return`<h3>Cloud Save</h3><p class="help">This game syncs to the cloud. Enter the code on another device to continue there. Keep it private: anyone with it can open, overwrite or delete this save.</p><p class="cloud-code">${formatCode(memo.code)}</p><div class="button-row"><button data-cloud="copy">Copy Code</button>${paused?'<button class="primary" data-cloud="compare">Choose Which Save to Keep</button>':'<button data-cloud="sync">Sync Now</button>'}</div><div class="button-row"><button data-cloud="stop">Stop Syncing on This Device</button><button data-cloud="delete">Delete Cloud Save</button></div>`;
  },
  ...
  async open(){
   ...
   if(decision==='forget'){stop('Your cloud save no longer exists, so syncing stopped. Your game is still saved here.');return;}
