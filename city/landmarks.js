...
export function landmarkPreview(d){if(!validLandmark(d))return '';const name=d.name.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));return L`<section class="landmark-preview"><img src="${landmarkURL(d.id)}" alt="${name} design image"><h3>${name}</h3><p>My landmark · ${d.type==='hotel'?L('Hotel'):L('Office Building')}</p></section>`;}
...
export function landmarkBounds(t,p,zoom,imageWidth,imageHeight){
 ...
 // p is the lot center. Seat the image's front base on the front half of the
 // isometric ground diamond instead of lifting its entire base above the center.
 return {x:p.x-w/2,y:p.y+w/4-h,width:w,height:h};
}
