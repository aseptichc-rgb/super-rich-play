...
export const COMPOUND_ASSETS = {
 stocks: {name:'Dividend stocks',icon:'↗',cost:100000,rate:.012,cycle:false,risk:'Safe',description:'Fixed dividends regardless of the economy. Slow, but you never lose.'},
 building: {name:'Rental building',icon:'🏙',cost:300000,rate:.02,cycle:true,risk:'Growth',description:'Rent follows the economy. More in a boom, less in a slump.'},
 resort: {name:'Hotel & Resort',icon:'🏝',cost:600000,rate:.04,cycle:true,risk:'Aggressive',shock:{chance:.06,loss:.3,label:'Typhoon · tourist slump'},description:'Big returns, but a 6% chance each month of losing 30% of the balance.'}
};
...
export function buyCompound(s,id){
 if(s.concept!=='rich-life'||!Object.hasOwn(COMPOUND_ASSETS,id))return {ok:false,msg:'Pick an asset to invest in.'};
 const d=COMPOUND_ASSETS[id],price=marketPrice(s,d.cost);if(s.money<price)return {ok:false,msg:'Save up a little more cash.'};
 const c=ensureCompound(s);s.money-=price;c.balances[id]+=d.cost;c.principal+=price;
 if(id!=='stocks')awardAssetFame(s,`compound:${id}`,d.cost,d.name+' acquired');
 return {ok:true,msg:d.name+' expanded! Monthly income +'+money(tieredIncome(d.cost,compoundRate(s,id)))};
}
...
export function settleCompound(s){
 ...
 for(const [id,d] of Object.entries(COMPOUND_ASSETS)){
  ...
  if(d.shock&&c.balances[id]>0&&hashRoll(s.seed,s.month+1,'compound-shock-'+id)<d.shock.chance){const loss=Math.round(c.balances[id]*d.shock.loss);c.balances[id]-=loss;c.lost=(c.lost||0)+loss;c.lastShock={month:s.month,id,loss,label:d.shock.label};s.log.unshift(`⚠ ${d.shock.label} · ${d.name} balance −${money(loss)}`);}
 }
 c.earned+=c.last;
}
