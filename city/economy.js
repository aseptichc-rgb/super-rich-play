...
export function economyReport(s){
 ...
 const label=market<1?'Market crash':{boom:'Boom',bust:'Bust',steady:'Steady'}[phase],trend=next>cycle+.005?'Rising':next<cycle-.005?'Falling':'Peak';
...
export function economyBanner(s){const f=marketFactor(s),r=economyReport(s);return `<section class="side-section" role="status"><h3>${f<1?'📉 '+(s.month-s.economy.start<s.economy.duration?'Market crash · Buy-the-dip window':'Recovery underway')+' · Prices −'+Math.round((1-f)*100)+'%':`Economy: ${r.label} · Income ${r.percent>0?'+':''}${r.percent}%`}</h3><p class="help">${f<1?'Stocks, property, companies, artwork and compound assets can be bought with cash at reduced prices. Business, rental and owner income drops 30%.':`A monthly business cycle swings rental, hotel and compound income by ±14% (${r.trend}). Every 5–10 years a market crash knocks asset prices down 20–50%.`}</p></section>`;}
