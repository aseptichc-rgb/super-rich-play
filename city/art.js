...
export const ARTWORKS={
 water_lilies:{name:'Water Lilies',original:'Nymphéas',artist:'Claude Monet',year:'1899–1926',price:12000000,annualRate:.035,rarity:'IMPRESSIONIST ICON',palette:['#547f78','#a9b987','#d3adbd'],description:'The signature Impressionist series capturing a moment of light and water.'},
 the_kiss:{name:'The Kiss',original:'Der Kuss',artist:'Gustav Klimt',year:'1907–1908',price:28000000,annualRate:.032,rarity:'GOLDEN PERIOD',palette:['#b7902d','#e2c765','#54452e'],description:'The Vienna Secession icon fusing gilded ornament with figures.'},
 girl_pearl:{name:'Girl with a Pearl Earring',original:'Girl with a Pearl Earring',artist:'Johannes Vermeer',year:'c. 1665',price:42000000,annualRate:.03,rarity:'DUTCH MASTER',palette:['#142c39','#d2a33e','#efe1c5'],description:'A Dutch Golden Age masterpiece remembered for its light and gaze.'},
 starry_night:{name:'The Starry Night',original:'The Starry Night',artist:'Vincent van Gogh',year:'1889',price:95000000,annualRate:.045,rarity:'POST-IMPRESSIONIST ICON',palette:['#183b62','#e6c64c','#5c87a4'],description:'The Post-Impressionist icon famous for its swirling night sky.'},
 night_watch:{name:'The Night Watch',original:'The Night Watch',artist:'Rembrandt van Rijn',year:'1642',price:110000000,annualRate:.028,rarity:'OLD MASTER',palette:['#2c2118','#9b7133','#b33f2f'],description:'The Baroque masterpiece that reinvented the group portrait with light and motion.'},
 mona_lisa:{name:'Mona Lisa',original:'Mona Lisa',artist:'Leonardo da Vinci',year:'c. 1503–1519',price:500000000,annualRate:.025,rarity:'ULTIMATE MASTERPIECE',palette:['#403b28','#7b7450','#b4925e'],description:'The priciest trophy art in the game world. A fictional listing unrelated to the real Louvre collection.'}
};
...
export function artQuote(s,id){
 ...
 if(!d)return'No artwork selected.';
 if(owned.some(item=>item.id===id))return'Already in your collection.';
 if(s.mode!=='sandbox'&&artFame(s)<(ART_FAME[id]||0))return`Reputation ${ART_FAME[id]} required (currently ${Math.round(artFame(s))})`;
 if(s.money<total(s,d))return`Cash ${money(total(s,d))} required`;
 return null;
}
...
export function buyArtwork(s,id){
 ...
 s.log.unshift(`🖼 ${d.artist} 〈${d.name}〉 won at auction · ${money(paid)} (fees included)`);s.log=s.log.slice(0,25);
 awardAssetFame(s,`art:${id}`,d.price,d.name+' won at auction');
 return{ok:true,msg:`${d.artist} 〈${d.name}〉 · now on display in your private gallery!`,paid};
}
