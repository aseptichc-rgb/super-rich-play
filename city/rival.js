...
const NAMES=['Kangho Baek','Jian Seo','Doyun Han','Yunseo Choi','Taeo Kang'];
...
export function advanceRival(s,playerWealth){
 ...
 if(s.month===r.nextBuy-1&&r.target<0){
  ...
  if(c!==undefined){r.target=c;const{x,y}=coords(c);s.log.unshift(`⚑ Rival ${r.name} · eyeing a ${TYPES[rivalTier(r)].name} lot at Street ${x+1}, Lot ${y+1}. Buying next month`);events.push({kind:'target',tile:c});}
 }else if(s.month>=r.nextBuy){
  const i=r.target,t=s.tiles[i];
  if(i>=0&&t&&t.terrain==='land'&&!t.type&&!t.owner){const type=rivalTier(r),{x,y}=coords(i);Object.assign(t,{type,owner:'rival',level:1,tree:false,constructionCost:developmentQuote(s,i,type).construction});r.tiles.push(i);r.acquired++;s.log.unshift(`⚑ Rival ${r.name} · broke ground on a ${TYPES[type].name} at Street ${x+1}, Lot ${y+1}`);events.push({kind:'bought',tile:i});}
  else if(i>=0){r.preempted++;s.prestige=Math.max(0,(s.prestige||0)+5);s.log.unshift('✦ You secured the lot before the rival · Reputation +5');events.push({kind:'preempted',tile:i});}
...
export function rivalSummary(s,wealth){
 ...
 return{name:r.name,wealth:r.wealth,ahead:wealth>=r.wealth,gap:wealth-r.wealth,share,buildings:r.tiles.length,target:r.target,nextBuy:r.nextBuy,acquired:r.acquired,preempted:r.preempted,label:wealth>=r.wealth*1.5?'Commanding lead':wealth>=r.wealth?'Narrow lead':wealth*1.5>=r.wealth?'Chasing':'Far behind'};
}
...
export function rivalCard(s,wealth){
 ...
 return `<section class="side-section rival-race"><h3>Tycoon Race <span>⚑</span></h3><div class="race-bar" role="img" aria-label="Me ${money(wealth)} vs ${v.name} ${money(v.wealth)}"><i style="width:${Math.round(v.share*100)}%"></i></div><div class="race-names"><span>Me <b>${money(wealth)}</b></span><span>${v.name} <b>${money(v.wealth)}</b></span></div><p class="help">${v.label} · Rival buildings: ${v.buildings}${v.target>=0?` · Eyeing <b>Street ${coords(v.target).x+1}, Lot ${coords(v.target).y+1}</b>`:` · Next purchase in ${Math.max(0,v.nextBuy-s.month)} months`}</p>${v.target>=0?`<button class="portfolio-link" data-rival-target="${v.target}">View Targeted Lot →</button>`:''}</section>`;
}
