...
const NAMES=['백강호','서지안','한도윤','최윤서','강태오'];
...
export function advanceRival(s,playerWealth){
 ...
 if(s.month===r.nextBuy-1&&r.target<0){
  ...
  if(c!==undefined){r.target=c;const{x,y}=coords(c);s.log.unshift(`⚑ 라이벌 ${r.name} · ${x+1}번가 ${y+1}번지 ${TYPES[rivalTier(r)].name} 부지를 노립니다. 다음 달 매입 예정`);events.push({kind:'target',tile:c});}
 }else if(s.month>=r.nextBuy){
  const i=r.target,t=s.tiles[i];
  if(i>=0&&t&&t.terrain==='land'&&!t.type&&!t.owner){const type=rivalTier(r),{x,y}=coords(i);Object.assign(t,{type,owner:'rival',level:1,tree:false,constructionCost:developmentQuote(s,i,type).construction});r.tiles.push(i);r.acquired++;s.log.unshift(`⚑ 라이벌 ${r.name} · ${x+1}번가 ${y+1}번지에 ${TYPES[type].name} 착공`);events.push({kind:'bought',tile:i});}
  else if(i>=0){r.preempted++;s.prestige=Math.max(0,(s.prestige||0)+5);s.log.unshift('✦ 라이벌보다 먼저 부지를 확보했습니다 · 명성 +5');events.push({kind:'preempted',tile:i});}
...
export function rivalSummary(s,wealth){
 ...
 return{name:r.name,wealth:r.wealth,ahead:wealth>=r.wealth,gap:wealth-r.wealth,share,buildings:r.tiles.length,target:r.target,nextBuy:r.nextBuy,acquired:r.acquired,preempted:r.preempted,label:wealth>=r.wealth*1.5?'압도적 선두':wealth>=r.wealth?'근소한 선두':wealth*1.5>=r.wealth?'추격 중':'크게 뒤처짐'};
}
...
export function rivalCard(s,wealth){
 ...
 return `<section class="side-section rival-race"><h3>부호 레이스 <span>⚑</span></h3><div class="race-bar" role="img" aria-label="나 ${money(wealth)} 대 ${v.name} ${money(v.wealth)}"><i style="width:${Math.round(v.share*100)}%"></i></div><div class="race-names"><span>나 <b>${money(wealth)}</b></span><span>${v.name} <b>${money(v.wealth)}</b></span></div><p class="help">${v.label} · 라이벌 건물 ${v.buildings}곳${v.target>=0?` · <b>${coords(v.target).x+1}번가 ${coords(v.target).y+1}번지</b>를 노리는 중`:` · 다음 매입 ${Math.max(0,v.nextBuy-s.month)}개월 뒤`}</p>${v.target>=0?`<button class="portfolio-link" data-rival-target="${v.target}">노리는 부지 보기 →</button>`:''}</section>`;
}
