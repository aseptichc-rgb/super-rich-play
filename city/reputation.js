...
export const PROPERTY_PARTIES={golf:{name:'Golf Club Party',cost:5000,stress:25,fame:20},resort:{name:'Resort Party',cost:10000,stress:40,fame:30}};
...
function gainFame(s,gain,label){
 ...
 const msg=`${label} · Reputation +${gain}`;s.log.unshift(msg);s.log=s.log.slice(0,25);return{ok:true,msg};
}
...
export function donate(s,amount){
 if(!Number.isSafeInteger(amount)||amount<1000)return{ok:false,msg:'Enter a whole-number donation of ₲1,000 or more.'};
 if(s.money<amount)return{ok:false,msg:'Not enough cash to donate.'};
 const total=(s.reputation?.donated||0)+amount;
 if(!Number.isSafeInteger(total))return{ok:false,msg:'Cumulative donation limit exceeded.'};
 const r=ensure(s),gain=Math.floor(total/1000)-Math.floor(r.donated/1000);
 s.money-=amount;r.donated=total;return gainFame(s,gain,`Community donation ${money(amount)}`);
}
export function mediaInterview(s){
 if(s.reputation?.lastInterview===s.month)return{ok:false,msg:'You already did an interview this month.'};
 ensure(s).lastInterview=s.month;return gainFame(s,INTERVIEW_FAME,'Press interview · My story of investing and giving');
}
export function hostPropertyParty(s,i){
 ...
 if(!party||t.owner!=='player'||t.tenure!=='buy')return{ok:false,msg:'Parties can only be hosted at a golf club or resort you own.'};
 if(s.reputation?.lastParties?.[t.type]===s.month)return{ok:false,msg:'You already hosted a party at this type of venue this month.'};
 if(s.money<party.cost)return{ok:false,msg:'Not enough cash to host the party.'};
 ...
 return gainFame(s,party.fame,`${party.name} hosted · ${money(party.cost)} · Stress −${relief}`);
}
export function propertyPartyPanel(s,i){
 ...
 return `<section class="owner-reputation"><h3>🥂 ${party.name}</h3><p>Invite guests to unwind and mingle.</p><p>Hosting cost ${money(party.cost)} · Stress up to −${party.stress} · Reputation +${party.fame}</p><button class="primary full" data-property-party="${i}" ${done||poor?'disabled':''}>${done?"This month's party held":poor?'Not enough cash to host':'Host a Party'}</button><p class="help">Once a month per venue type (golf club / resort). Owning several venues of the same type still shares one hosting slot.</p></section>`;
}
...
export function publicActivitiesPanel(s){
 ...
 return `<section class="owner-reputation public-activities"><h3>Giving & Public Life</h3><p>Total donated <b>${money(r?.donated||0)}</b> · Cash on hand ${money(Math.round(s.money))}</p><p>Donations fund community education and healthcare. Every cumulative ₲1,000 earns 1 reputation point, and the amount donated reduces your cash and net worth.</p><div class="button-row">${DONATION_AMOUNTS.map(n=>`<button data-donate="${n}" ${s.money<n?'disabled':''}>Donate ${money(n)} · +${n/1000} pts</button>`).join('')}</div><label class="field-label" for="donation-amount">Custom donation amount (₲)<input id="donation-amount" type="number" min="1000" step="1" value="1000"></label><button data-action="donate-custom" class="primary full">Donate Entered Amount</button><h3>Press Interview</h3><p>Share your story of investing and giving. Free, once a month · Reputation +${INTERVIEW_FAME}</p><button data-action="media-interview" class="full" ${done?'disabled':''}>${done?'Interview done this month':'Give a Press Interview'}</button></section><p class="help">Big-ticket property, mansion, vehicle, yacht and art purchases: ${[...ASSET_FAME].reverse().map(t=>`${money(t.cost)} or more: +${t.fame} pts`).join(' / ')}. Small business acquisitions earn at least +10 pts. The same asset only counts reputation gained above its previous best, and stock trades do not qualify. Existing collections are not credited retroactively.</p>`;
}
