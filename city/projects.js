...
export const CREW_HOURS=15;
export const PATHS={
 film:{name:L('Indie Film Production'),icon:'▣',tag:L('Executive produce → Premiere → Rights'),cost:100000,hours:80,sale:30000,royalty:4000,richOnly:true,desc:L('Finish your own indie film with a crew. Your crew handles production automatically. You earn a completion fee plus 24 months of rights income.')},
 couture:{name:L('Designer Collection'),icon:'✧',tag:L('Art direction → Exhibition → License'),cost:50000,hours:60,sale:15000,royalty:2000,richOnly:true,desc:L('Create a limited-edition design collection with a master atelier. Your atelier handles production automatically. You earn a completion fee plus 24 months of license income.')},
...
export function startProject(s,type,style,name){
 ...
 return{ok:true,msg:PATHS[type].richOnly?L('Production started! Your crew works automatically each month.'):L('Project started! Allocate creative hours in Time & Life.')};
}
export function projectReport(s){const projects=s.projects||[],working=projects.filter(p=>p.status==='working'),multiplier=reputationSummary(s).creativeMultiplier*(s.tiles?.[s.journey?.workroom]?.owner==='player'&&s.tiles?.[s.journey?.workroom]?.type==='atelier'?1.15:1),hours=(s.concept==='rich-life'?0:(s.plan.create||0))*multiplier/Math.max(1,working.length);let income=0;const completions=[],progress={};
 for(const p of projects){if(p.status==='working'){progress[p.id]=PATHS[p.type].richOnly?CREW_HOURS*multiplier:hours;if(p.progress+progress[p.id]>=p.hours){income+=p.sale;completions.push(p.id);}}if(p.status==='earning'&&p.monthsLeft>0)income+=p.royalty;}
 return{income,completions,hours,crewHours:CREW_HOURS*multiplier,progress,working:working.length,royalties:projects.filter(p=>p.status==='earning').reduce((n,p)=>n+p.royalty,0)};
}
export function advanceProjects(s){const report=projectReport(s);for(const p of s.projects||[]){if(p.status==='working'){p.progress=Math.min(p.hours,p.progress+report.progress[p.id]);if(p.progress>=p.hours){p.status=p.royalty?'earning':'complete';p.monthsLeft=p.royalty?24:0;s.log.unshift(L`✦ ${p.name} complete! ${p.royalty?L`${p.royalty}G per month from next month`:L`Sale/delivery fee ${p.sale}G`}`);}}else if(p.status==='earning'){p.monthsLeft--;if(p.monthsLeft<=0)p.status='complete';}}return report;}
