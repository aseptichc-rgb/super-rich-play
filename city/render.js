...
export function createRenderer(canvas,getState,getAnalysis,onTile,onHover){
 ...
 function render(time,phase=0){
  ...
  if(collection.includes('penthouse')){
   ...
   const p=screen(3.25,20.45,z+38);circle(p,12,'#fff1bd88');ctx.font=`bold ${12*zoom}px sans-serif`;ctx.textAlign='center';ctx.fillStyle='#60451c';ctx.fillText(L`♛ PRIVATE ESTATE · ${d.floors} floors · ${d.rooms} rooms`,p.x,p.y+4*zoom);
  }
