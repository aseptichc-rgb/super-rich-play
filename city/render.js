...
export function createRenderer(canvas,getState,getAnalysis,onTile,onHover){
 ...
 // Zooming keeps the map point under the cursor (or the view center) in place, so the big map doesn't drift out of view.
 function zoomTo(next,at={x:w*.5,y:h*.5}){next=Math.max(.45,Math.min(2.3,next));const r=next/zoom,dx=at.x-w*.5,dy=at.y-h*.5;panX=dx-(dx-panX)*r;panY=dy-(dy-panY)*r;zoom=next;}
