...
import {SIZE,coords,neighbors,TYPES,footprintCells} from './engine.js';
...
export function createWorldLife(ctx,screen,box,circle,line,getState,getAnalysis){
 let actors=[],roads=[],previousState=null,last=0,buckets=new Map(),particles=[],clock=0,hero=null;
 ...
 function reset(){previousState=getState();roads=[...getAnalysis().connected];actors=[];particles=[];for(let n=0;n<34;n++){const i=roads[n*13%roads.length],p=coords(i),a={id:n,x:p.x,y:p.y,path:[i],step:0,trips:0,wait:0,car:n%7===0,color:['#d88264','#e4bb5d','#598c91','#b68aae','#687d5a'][n%5]};destination(a);actors.push(a);}const p=coords(roadAt(9*SIZE+9));hero={id:100,x:p.x,y:p.y,path:[],step:0,color:'#ee986d',wait:0};actors.push(hero);}
 function update(time){if(previousState!==getState())reset();if(roads.length!==getAnalysis().connected.size)roads=[...getAnalysis().connected];const dt=Math.min(.06,(time-last)/1000||0);last=time;clock=time;buckets=new Map();
  for(const a of actors){if(!reduce.matches){if(a.wait>0)a.wait-=dt;else if(a.step<a.path.length-1){const dest=coords(a.path[a.step+1]),dx=dest.x-a.x,dy=dest.y-a.y,d=Math.hypot(dx,dy),move=dt*(a.car?2.4:a===hero?2.8:1.05);if(d<=move){a.x=dest.x;a.y=dest.y;a.step++;}else{a.x+=dx/d*move;a.y+=dy/d*move;}}else if(a!==hero){a.wait=1+(a.id%4)*.35;destination(a);}}
 ...
 function person(a,time){const x=a.x+.5,y=a.y+.5,moving=a.step<a.path.length-1,bob=moving?Math.sin(time*.013+a.id)*1.1:0;
  ...
  if(a===hero&&flexState(getState()).owned.includes('sportscar')){const model=selectedModel(getState(),'sportscar');drawMapCar(ctx,screen,x,y,model,a.heading||1);const p=screen(x,y,29);ctx.font='bold 10px sans-serif';ctx.textAlign='center';ctx.fillStyle='#765322';ctx.fillText('✦ '+model.name,p.x,p.y);return;}
  ...
  if(a===hero){const p=screen(x,y,29);ctx.font='bold 10px sans-serif';ctx.textAlign='center';ctx.fillStyle='#264a3b';ctx.fillText(L('Me'),p.x,p.y);}
 }
 ...
 function drawFront(time,phase=0){
  ...
  if(!reduce.matches){for(let n=0;n<4;n++){const pos=((time*.000035+n*.08)%1),p=screen(21+Math.sin(pos*6)*.5,2+pos*20,70+n*3);line({x:p.x-4,y:p.y+Math.sin(time*.007)*2},p,'#64828c',1);line(p,{x:p.x+4,y:p.y+Math.sin(time*.007)*2},'#64828c',1);}}
 }
 function burst(i,text,kind='build'){if(i<0)i=9*SIZE+9;const{x,y}=coords(i);particles.push({x,y,start:clock,life:2400,text,color:kind==='loss'?'#b95249':'#376b49'});if(!reduce.matches)for(let n=0;n<30;n++)particles.push({x,y,start:clock,life:1300+n%5*150,dx:Math.sin(n*2.4)*(35+n),dy:-20-Math.cos(n*1.5)*55,color:['#efc965','#ed9e75','#8dc38d','#92c9d2'][n%4]});}
