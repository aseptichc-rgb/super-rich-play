...
import {SIZE,coords,neighbors,TYPES,footprintCells,mapOffset} from './engine.js';
...
export function createWorldLife(ctx,screen,box,circle,line,getState,getAnalysis,getAssetTop=()=>null){
 ...
 function drawFront(time,phase=0){
  ...
  for(const{t,i}of owned){const{x,y}=coords(i),r=getAnalysis().reports[i];if(!r||t.type==='plot')continue;const p=getAssetTop(i)||(['golf','resort'].includes(t.type)?screen(x+(t.footprint?.width||1)/2,y+(t.footprint?.height||1)/2,t.type==='golf'?32:43):screen(x+.5,y+.5,57+t.level*10)),pulse=reduce.matches?1:.7+Math.sin(time*.003+i)*.3;
  ...
  if(!reduce.matches){for(let n=0;n<4;n++){const pos=((time*.000035+n*.08)%1),o=mapOffset(getState()),p=screen(o+21+Math.sin(pos*6)*.5,o+2+pos*20,70+n*3);line({x:p.x-4,y:p.y+Math.sin(time*.007)*2},p,'#64828c',1);line(p,{x:p.x+4,y:p.y+Math.sin(time*.007)*2},'#64828c',1);}}
 }
 function burst(i,text,kind='build'){if(i<0)i=homeRoad();const{x,y}=coords(i);particles.push({x,y,start:clock,life:2400,text,color:kind==='loss'?'#b95249':'#376b49'});if(!reduce.matches)for(let n=0;n<30;n++)particles.push({x,y,start:clock,life:1300+n%5*150,dx:Math.sin(n*2.4)*(35+n),dy:-20-Math.cos(n*1.5)*55,color:['#efc965','#ed9e75','#8dc38d','#92c9d2'][n%4]});}
